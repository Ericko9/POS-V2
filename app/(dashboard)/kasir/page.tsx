...
interface Kasir { id: string; nama: string; username: string; email: string | null; noHp: string | null; aktif: boolean; catatan: string | null; role: string; createdAt: string }
...
export default function KasirPage() {
  ...
  const [form, setForm] = useState({ nama: "", username: "", password: "", email: "", noHp: "", catatan: "", role: "KASIR" })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (json.success) { setShowForm(false); setEditId(null); setForm({ nama: "", username: "", password: "", email: "", noHp: "", catatan: "", role: "KASIR" }); fetchData() }
    else alert(json.message)
    setSaving(false)
  }

  const handleEdit = (k: Kasir) => { setEditId(k.id); setForm({ nama: k.nama, username: k.username, password: "", email: k.email || "", noHp: k.noHp || "", catatan: k.catatan || "", role: k.role }); setShowForm(true) }
  ...
  return (
    <div className="space-y-6 pt-12 lg:pt-0">
      <div className="page-header">
        <div><h1 className="page-title flex items-center gap-2"><Users className="w-7 h-7 text-primary" /> Manajemen Staf</h1><p className="page-subtitle">{data.length} staf</p></div>
        <button onClick={() => { setShowForm(true); setEditId(null); setForm({ nama: "", username: "", password: "", email: "", noHp: "", catatan: "", role: "KASIR" }) }} className="btn-primary"><Plus className="w-4 h-4" /> Tambah Staf</button>
      </div>

      {showForm && (
        <form onSubmit={handleSubmit} className="card max-w-lg space-y-3">
          <h2 className="font-semibold">{editId ? "Edit" : "Tambah"} Staf</h2>
      ...
      {loading ? <div className="card text-center py-12 text-muted">Memuat...</div> : (
        <div className="table-container">
          <table className="table-base">
            <thead><tr><th>Nama</th><th>Username</th><th>Role</th><th>Kontak</th><th>Status</th><th>Aksi</th></tr></thead>
            <tbody>
              {data.map(k => (
                <tr key={k.id}>
                  <td className="font-medium">{k.nama}</td>
                  <td className="font-mono text-sm">{k.username}</td>
                  <td><span className={`badge ${k.role === "KURIR" ? "badge-info" : "badge-primary"}`}>{k.role}</span></td>
